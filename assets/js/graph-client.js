/**
 * Client-side graph visualization for Jekyll Garden
 * Generates graph data on the fly and creates interactive visualizations
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Helper function to remove code blocks from content
  ...
  const extractWikilinks = (content) => {
    ...
    while ((match = wikilinksRegex.exec(contentWithoutCodeBlocks)) !== null) {
  ...
  // Generate graph data from notes and books
  const generateGraphData = async () => {
    try {
      ...
      if (searchIndexJson) {
        ...
        // Wait for search index to be generated
        searchIndex = await new Promise((resolve) => {
      ...
      const nodes = documents.map(doc => ({
        ...
        url: doc.url,
        type: doc.type,
        excerpt: doc.content.length > 150 ? doc.content.substring(0, 150) + '...' : doc.content
      }));
      
      // Create a map of document IDs to their URLs for faster lookup
      const docUrlMap = {};
      documents.forEach(doc => {
        docUrlMap[doc.id] = doc.url;
      });
      
      // Create a map of document titles to their IDs for faster lookup
      const docTitleMap = {};
      documents.forEach(doc => {
        docTitleMap[doc.title.toLowerCase()] = doc.id;
      });
      
      // Create links by fetching and parsing each document
      const links = [];
      ...
      for (const doc of documents) {
        ...
        const contentMatch = docContent.match(/<div class="(?:note|book)-content">([\s\S]*?)<\/div>/);
        ...
        for (const wikilink of wikilinks) {
          let targetCollection = 'notes';
          let displayText = wikilink;
          
          // Check if link starts with Book: prefix
          if (wikilink.startsWith('Book:')) {
            targetCollection = 'books';
            if (wikilink.startsWith('Book: ')) {
              displayText = wikilink.substring(6).trim(); // Remove "Book: " with space
            } else {
              displayText = wikilink.substring(5).trim(); // Remove "Book:" without space
            }
          }
          
          // Create slug from display text
          const slug = createSlug(displayText);
          
          // Find target document by slug
          const targetDoc = documents.find(d => d.id === slug);
          if (!targetDoc) continue;
          
          // Create link
          const linkKey = `${doc.id}-${targetDoc.id}`;
